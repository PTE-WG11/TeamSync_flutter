...
// 避免使用 dynamic
// 好
final User user = await repository.getUser();
// 不好
final dynamic user = await repository.getUser();

// 使用扩展函数增强可读性
extension DateTimeExtension on DateTime {
  String toDisplayString() => DateFormat('yyyy-MM-dd').format(this);
}
```
...
- **编码习惯**：强调使用 `const` 构造函数提升性能，变量尽量使用 `final` 修饰，严禁滥用 `dynamic` 类型，并鼓励使用 `extension` 增强代码可读性。
